...
fn token_usage_exceeded_triage_result() -> RunResult {
    RunResult {
        ...
        human_review_reason: None,
        blocked_reason: Some(
            "LLM triage could not run because token usage or provider quota was exceeded. The configured agent platform did not use deterministic fallback triage for this issue."
                .to_string(),
        ),
    }
}
...
fn developer_pull_request_body(
    ...
    format!(
        "Closes #{}\n\n## Summary\n{}\n\n## Changed Files\n{}\n\n## Tests\n{}\n\nGenerated by {} implementation job `{}`.\n\n<!-- donkeyspace-generated -->",
        issue_number(&running_job.input).unwrap_or(0),
        result.summary,
        changed_files
            .iter()
