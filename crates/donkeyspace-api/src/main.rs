...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let github = env::var("DONKEYSPACE_GITHUB_TOKEN")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .map(GitHubClient::new)
        .transpose()?;
    let github_token_owner = if let Some(client) = &github {
        Some(client.authenticated_login().await?)
    } else {
        None
    };
...
fn start_github_poller(state: Arc<AppState>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if state.github.is_none() {
        return Err("github polling requires DONKEYSPACE_GITHUB_TOKEN".into());
    }
    ...
    tokio::spawn(async move {
        loop {
            let client = state.github.as_ref().expect("polling client was validated");
...
async fn persist_issue_webhook(
    ...
    let managed_resource = if let Some(comment) = &payload.comment {
        ...
        is_projected_work_item(&payload.issue.body)
            || github_managed_resource_exists(
                pool,
                repository_id,
                "issue",
...
async fn authorize_engagement(
    ...
    let rule = state.policy.workflow.engagement.rule(gate);
...
mod tests {
    use super::{
        ...
        GitHubLabel, GitHubOwner, GitHubRepository, JobRecord, authorize_engagement, can_retry_job,
        ...
        permission_rank, should_queue_reviewer, should_queue_triage,
    };
