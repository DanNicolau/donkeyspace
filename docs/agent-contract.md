...
- Triage jobs receive issue title, body, labels, the latest relevant comment,
  and a prepared repository checkout. The optional OpenAI-compatible triage
  path receives bounded excerpts instead of invoking an external agent command.
- Developer jobs receive issue context, policy checks, and a writable repository checkout path.
- Reviewer jobs receive issue context, PR metadata, a diff summary, changed
  files, and patch context.
...
Reviewer agents review donkeyspace-managed PRs after `pull_request` webhooks. A reviewer result of `needs_changes` posts a PR comment and keeps the issue in `pr_open`; it does not automatically requeue implementation in the default lifecycle. A lifecycle plugin may define its own bounded feedback edges between tasks.

A reviewer result of `reviewed` also posts a PR comment and leaves the issue in `pr_open`; it is an audit signal, not automatic approval or merge authority.
...
- Contradictory result, such as `outcome: ready` with `confidence: low`: reject
  the result and fail the job closed.
